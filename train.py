...
class gaga_lightning(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.generator = Generator(self.hparams, cmin=self.hparams.min, cmax=self.hparams.max)
        self.generator_acc = Generator(self.hparams, cmin=self.hparams.min, cmax=self.hparams.max)
        ...
        self.z = torch.randn(1000, int(self.hparams.batchsize), self.hparams.z_dim)
    ...
    def train_dataloader(self):
        ...
        for dataset in self.train_dataset[1:]:
            phase, _, _ = phsp.load(os.path.join(self.hparams.train_dataroot,dataset))
            array_list.append(phase)
        array = np.concatenate(array_list, axis=0)[:, [0, 1, 2, 4, 5, 6]]
        self.hparams.mean = array.mean(axis=0)
        self.hparams.std = array.std(axis=0)
        self.hparams.min = array.min(axis=0)
        self.hparams.max = array.max(axis=0)

        #array = ((array-self.hparams.min)/(self.hparams.max-self.hparams.min)-0.5)/0.5
        array = (array-self.hparams.mean)/self.hparams.std

        self.data_size = array.shape[0]
        print(array.shape)
        dataloader  = DataLoader(array,
    ...
    def val_dataloader(self):
        ...
        print(phase.shape)
        dataloader = DataLoader(phase[:, [0, 1, 2, 4, 5, 6]], shuffle=False, pin_memory=True, batch_size=int(self.hparams.batchsize))
        return dataloader
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        #real_sample[:, 0] = (real_sample[:, 0] -0.04)/ 0.02
        #real_sample[:, 1] = real_sample[:,1]/100
        #real_sample[:, 2] = real_sample[:,2]/100
        #real_sample[:, 5] = real_sample[:, 5] #+0.75
        ...
        if optimizer_idx == 1:
            ...
            self.fake_sample = self(z)

            pred = self.discriminator(self.fake_sample)
            ...
            fake_pred = self.discriminator(self.fake_sample.detach())
    ...
    def validation_step(self, batch, batch_idx):
        #real_sample = ((batch-self.hparams.min.cuda())/(self.hparams.max.cuda()-self.hparams.min.cuda())-torch.Tensor([0.5]).cuda())/torch.Tensor([0.5]).cuda()
        real_sample = batch#(batch*self.hparams.std.cuda())+self.hparams.mean.cuda()
        #real_sample[:, 0] = (real_sample[:, 0] -0.04)/ 0.02
        #real_sample[:,1] = real_sample[:,1]/100
        #real_sample[:,2] = real_sample[:,2]/100
        ...
        fake_sample = (self.generator_acc(z)*self.hparams.std.cuda())+self.hparams.mean.cuda()

        #fake_sample[:, 0] = fake_sample[:, 0]*0.02 + 0.04
        #fake_sample[:,1] = fake_sample[:,1]*100
        #fake_sample[:,2] = fake_sample[:,2]*100
        #fake_sample[:, 5] = fake_sample[:, 5] #-0.75

        return {'fake_samples': fake_sample, 'real_samples':real_sample}

    def validation_end(self, outputs):
        fake_samples = torch.reshape(torch.stack([x['fake_samples'] for x in outputs]), (-1,6))
        real_samples = torch.reshape(torch.stack([x['real_samples'] for x in outputs]), (-1,6))
        ...
        return{'val_loss':torch.Tensor([loss]), 'log':log, 'progress_bar':log }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_dataroot', type=str, default='data\\train_full')
    parser.add_argument('--valid_dataroot', type=str, default='data\\valid')
    ...
    parser.add_argument('--d_norm', type=bool, default=False, help='use norm yes no')
